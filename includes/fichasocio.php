...
if ($nivel_admin OR ($nivel_distrito AND $nivel_distrito_id==$idd) OR ($nivel_club_id==$idc)){echo $row['direccion']."<br />";}
if ($nivel_admin OR ($nivel_distrito_id==$idd) OR ($nivel_club_id==$idc)){echo "Celular: ".$row['celular']."<br />";}
if ($nivel_admin OR ($nivel_distrito AND $nivel_distrito_id==$idd) OR ($nivel_club AND $nivel_club_id==$idc)){echo "Fijo: ".$row['telefono']."<br />";}
if ($nivel_admin OR ($nivel_distrito_id==$idd) OR ($nivel_club_id==$idc)){setlocale(LC_ALL, 'es_ES'); echo "Fecha de Nacimiento: ".strftime ("%d %B %Y", strtotime($row['fecha_de_nacimiento']))."<br />";}
?>
&lt;listado de cargos&gt;
